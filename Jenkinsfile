pipeline {
  ...
  stages {
    ...
    stage('Plan') {
      steps {
        withCredentials([string(credentialsId: 'AWS_ACCESS_KEY', variable: 'aws_access_key'), 
          ...
          dir('Terraform') {
            sh 'cd /home/ubuntu/TF_WL5 && sudo terraform plan -out plan.tfplan -var="aws_access_key=${aws_access_key}" -var="aws_secret_key=${aws_secret_key}"' 
          }
        }
      }
    }
    stage('Apply') {
      steps {
        dir('Terraform') {
          sh 'cd /home/ubuntu/TF_WL5 && sudo terraform apply plan.tfplan' 
        }
      }
    }
  }
